  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight font-sans">
              Admin System & Security Audit Ledger
            </h1>
            <TactileBadge variant="blue" size="sm">Immutable Records</TactileBadge>
          </div>
          <p className="text-xs text-slate-500 font-mono mt-1">
            System-wide immutable decision logs, user activities, and evidence citations
          </p>
        </div>
      </div>
      ...
      <div className="w-full">
        <RecessedInput
          placeholder="Filter audit logs by action, entity ID, or evidence standard..."
          value={query}
      ...
      <div className="bg-white rounded-2xl border border-slate-200/90 shadow-xs overflow-hidden font-mono text-xs">
        <div className="p-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
          <span className="font-bold text-slate-900 font-sans">System Audit Ledger</span>
          <span className="text-slate-500">{filtered.length} Entries Recorded</span>
        </div>
